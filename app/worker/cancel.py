...
def request_cancel(redis_client: redis.Redis, job_id: str) -> None:
    """Set a cancellation flag for the given job."""
    redis_client.set(f"{CANCEL_KEY_PREFIX}{job_id}", "1", ex=CANCEL_TTL)


def is_cancelled(redis_client: redis.Redis, job_id: str) -> bool:
    """Check if a cancellation has been requested for the given job."""
    return redis_client.exists(f"{CANCEL_KEY_PREFIX}{job_id}") > 0


def clear_cancel(redis_client: redis.Redis, job_id: str) -> None:
    """Remove the cancellation flag for the given job."""
    redis_client.delete(f"{CANCEL_KEY_PREFIX}{job_id}")
